...
import { getCityGrapData, getCityComplianceReport } from "@/lib/city-mock-data";
...
export default function CompliancePage() {
  ...
  useEffect(() => {
    setLoading(true);
    setReport(null);
    fetchAPI<GRAPStatus>(`/api/v1/compliance/grap?city=${grapCity}`)
      .then(setGrap)
      .catch(() => {
        setGrap(getCityGrapData(grapCity));
      })
  ...
  const generateReport = async () => {
    ...
    try { setReport(await fetchAPI<ComplianceReport>(`/api/v1/compliance/report?city=${grapCity}`)); }
    catch {
      setReport(getCityComplianceReport(grapCity));
    } finally { setReportLoading(false); setLoadingStep(0); }
  };
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="pt-4">
                ...
                {reportLoading && (
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center gap-3"><div className={`w-2.5 h-2.5 rounded-full transition-all ${loadingStep >= 1 ? "bg-[var(--entity-good)] scale-110" : "bg-muted"}`}/><span className={`text-[13px] ${loadingStep >= 1 ? "text-foreground" : "text-muted-foreground"}`} style={{fontVariationSettings:"'wght' 520"}}>collecting sensor + weather data...</span></div>
                    <div className="flex items-center gap-3"><div className={`w-2.5 h-2.5 rounded-full transition-all ${loadingStep >= 2 ? "bg-[var(--entity-forecast)] scale-110" : "bg-muted"}`}/><span className={`text-[13px] ${loadingStep >= 2 ? "text-foreground" : "text-muted-foreground"}`} style={{fontVariationSettings:"'wght' 520"}}>running anomaly + attribution analysis...</span></div>
                    <div className="flex items-center gap-3"><div className={`w-2.5 h-2.5 rounded-full transition-all ${loadingStep >= 3 ? "bg-[var(--entity-moderate)] scale-110" : "bg-muted"}`}/><span className={`text-[13px] ${loadingStep >= 3 ? "text-foreground" : "text-muted-foreground"}`} style={{fontVariationSettings:"'wght' 520"}}>generating enforcement recommendations...</span></div>
                  </div>
                )}
                {!reportLoading && !report && <p className="mt-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider">runs full multi-agent analysis pipeline</p>}
              </div>
